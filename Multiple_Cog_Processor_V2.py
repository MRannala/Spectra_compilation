...
# ALSWAYS COMES OUT FALSE EVENTUALLY
# ADD TEST SUB ROUTINE HERE!    
def find_X(data, var):
...
def make_output_file(filename, df):
            ...
            df = " "*6 + df.to_string(header = False).replace("\n", "\n      ")
            
            # write to file
            ofile.write(df + '\n')
            
            # Write number of photons/s/g
            ofile.write('C Ph/s/g/:       {0}'.format(total))
...
if __name__ == '__main__':
    ...
    for i in cog_files:
        ...
        if find_X(data,'define') == False:
            ...
            # skip cog file if no photon spectrum
            next(cog_files)
